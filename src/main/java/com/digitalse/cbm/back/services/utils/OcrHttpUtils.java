...
public class OcrHttpUtils {
    ...
    static public void sendId(List<Long> ids) throws IOException {
        ...
        try {
            ...
            /*
             * //rObject(URL_OCR, String.class) JsonNode jsonNode = new
             * ObjectMapper().readTree(); //System.out.println(jsonNode.toString());
             * RFMilitar militarFound = new
             * RFMilitar(jsonNode.get("num_matricula").asText(),
             * jsonNode.get("nom_completo").asText(), jsonNode.get("nom_guerra").asText(),
             * jsonNode.get("dsc_cargo").asText()); HttpURLConnection connection =
             ...
             * DataOutputStream(connection.getOutputStream())) { byte[] input = new
             * Gson().toJson(ids).getBytes("utf-8"); outputStream.write(input, 0,
             * input.length);
             */
